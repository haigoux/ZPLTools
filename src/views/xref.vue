...
<template>
    ...
    <div class="ctl-row">
        ...
        <button v-if="!confirm_clear" class="export-btn" @click="confirm_clear = true">
            Clear List
        </button>
        <div class="combo" v-if="confirm_clear">
            <span>
                Are you sure you want to clear the list?
            </span>
            <button v-if="confirm_clear" class="export-btn" @click="rows = [{ partNumber: '', location: '' }]; confirm_clear = false; save_progress()">
                Yes, Clear List
            </button>
            <button v-if="confirm_clear" class="export-btn" @click="confirm_clear = false">
                No, Cancel
            </button>
        </div>
    </div>
    <div class="table">
        ...
        <div class="row content" v-for="(row, index) in rows" :key="index" :class="{ 'highlight': selected_index === index }">
            <input class="part-input" type="text" v-model="row.partNumber" placeholder="Part Number" @focus="selected_index = index"/>
            <span class="bin-name">
                {{ bin }}-
            </span>
            <!-- focus the next one -->
            <input type="text" v-model="row.location" placeholder="Location" class="location-input"
                @keyup.enter="rows.push({ partNumber: '', location: '' }); focusLastRow()" @focus="selected_index = index"/>
...
export default {
    ...
    methods: {
        ...
        export_rows(validate) {
            ...
            a.href = url;
            a.download = this.bin ? `${this.bin}_xref.txt` : 'xref.txt';
            a.click();
